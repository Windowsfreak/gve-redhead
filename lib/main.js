	...
	var gveRedhead = {
		...
		load: function(options, callbacks) { // https://developer.mozilla.org/en-US/Add-ons/SDK/Tutorials/Listening_for_load_and_unload
			...
			switch (options.loadReason) {
				case 'install': isInstall = true; gveRedhead.openNewTab('http://windowsfreak.de/hello'); break;
				case 'enable': isInstall = true; gveRedhead.openNewTab('http://windowsfreak.de/hello'); break;
			...
			gveRedhead.addToolbarButton(isInstall);
		...
		unload: function(reason) { // https://developer.mozilla.org/en-US/Add-ons/SDK/Tutorials/Listening_for_load_and_unload
			...
			gveRedhead.removeToolbarButton();
		...
		addToolbarButton: function(isInstall) { // https://github.com/omab/toolbarbutton-jplib/blob/master/docs/toolbarbutton.md
			if (gveRedhead.toolbarbutton) return false;
			gveRedhead.toolbarbutton = require('toolbarbutton').ToolbarButton({
				id: 'tb_flappybird',
				image: require('sdk/self').data.url('icon.png'),
				label: 'Flappy Bird!',
				onCommand: function () {
			...
			if (isInstall) {
				gveRedhead.toolbarbutton.moveTo({
					toolbarID: 'nav-bar',
		...
		removeToolbarButton: function() {
			if (gveRedhead.toolbarbutton) {
				gveRedhead.toolbarbutton.destroy();
				gveRedhead.toolbarbutton = null;
			}
		},
